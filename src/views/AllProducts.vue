    ...
    <details class="group [&_summary::-webkit-details-marker]:hidden">
      ...
      <div class="z-50 group-open:absolute group-open:start-0 group-open:top-auto group-open:mt-2">
        <div class="w-72 sm:w-96 rounded border border-gray-200 bg-white">
          <form @submit.prevent="Filters" class="border-t border-gray-200 p-4">
            ...
            <div class="flex justify-between w-full mt-5">
              <label for="Category" class="flex items-center w-full gap-2">
                ...
                >
                <option value="" class="text-center" disabled selected> Select Category </option>
              <option v-for="category in categoris" class="text-center" :value="category.name">{{ category.name }}</option>
              </select>
              </label>
            </div>
            <button type="submit" class="mt-5 w-full text-center bg-indigo-700 h-10 rounded text-white">Filter</button>
          </form>
        </div>
      </div>
    </details>
